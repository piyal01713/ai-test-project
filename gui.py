...
class LynkApp(tk.Tk):
    ...
    def create_widgets(self):
        # --- Top Mode Selection ---
        self.mode_frame = ttk.Frame(self, padding=10)
        self.mode_frame.pack(fill=tk.X)

        self.mode_var = tk.StringVar(value="sender")
        
        ttk.Label(self.mode_frame, text="Select Mode:", style="Header.TLabel").pack(side=tk.LEFT, padx=(0, 10))
    ...
    def build_sender_frame(self):
        ttk.Label(self.sender_frame, text="Network Configuration", style="Header.TLabel").grid(row=0, column=0, columnspan=4, sticky=tk.W, pady=(0, 10))

        ttk.Label(self.sender_frame, text="Host IP:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.sender_ip = ttk.Entry(self.sender_frame, width=20)
        self.sender_ip.insert(0, "0.0.0.0")
        self.sender_ip.grid(row=1, column=1, sticky=tk.W, pady=5, padx=(5, 15))

        ttk.Label(self.sender_frame, text="Port:").grid(row=1, column=2, sticky=tk.W, pady=5)
        self.sender_port = ttk.Entry(self.sender_frame, width=10)
        self.sender_port.insert(0, "8080")
        self.sender_port.grid(row=1, column=3, sticky=tk.W, pady=5, padx=5)

        ttk.Label(self.sender_frame, text="File to Send:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        self.sender_entry.grid(row=2, column=1, columnspan=3, sticky=tk.W, pady=5, padx=5)
        
        ttk.Button(self.sender_frame, text="Browse", command=self.browse_file).grid(row=2, column=4, sticky=tk.W, pady=5, padx=5)

        ttk.Button(self.sender_frame, text="Start Hosting", command=self.start_hosting).grid(row=3, column=0, columnspan=5, pady=(15, 0))

    def build_receiver_frame(self):
        ttk.Label(self.receiver_frame, text="Network Configuration", style="Header.TLabel").grid(row=0, column=0, columnspan=4, sticky=tk.W, pady=(0, 10))

        ttk.Label(self.receiver_frame, text="Host IP:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.receiver_ip = ttk.Entry(self.receiver_frame, width=20)
        self.receiver_ip.insert(0, "127.0.0.1")
        self.receiver_ip.grid(row=1, column=1, sticky=tk.W, pady=5, padx=(5, 15))

        ttk.Label(self.receiver_frame, text="Port:").grid(row=1, column=2, sticky=tk.W, pady=5)
        self.receiver_port = ttk.Entry(self.receiver_frame, width=10)
        self.receiver_port.insert(0, "8080")
        self.receiver_port.grid(row=1, column=3, sticky=tk.W, pady=5, padx=5)

        ttk.Label(self.receiver_frame, text="Save To:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        self.receiver_entry.grid(row=2, column=1, columnspan=3, sticky=tk.W, pady=5, padx=5)
        
        ttk.Button(self.receiver_frame, text="Browse", command=self.browse_folder).grid(row=2, column=4, sticky=tk.W, pady=5, padx=5)

        ttk.Button(self.receiver_frame, text="Connect & Receive", command=self.start_receiving).grid(row=3, column=0, columnspan=5, pady=(15, 0))

    def build_status_frame(self):
        ttk.Label(self.status_frame, text="Transfer Status", style="Header.TLabel").pack(anchor=tk.W, pady=(0, 5))

        self.status_lbl = ttk.Label(self.status_frame, text="Status: Idle", font=("Helvetica", 10, "italic"))
        self.status_lbl.pack(anchor=tk.W, pady=2)

        self.progress_var = tk.DoubleVar()
        self.progress_bar = ttk.Progressbar(self.status_frame, variable=self.progress_var, maximum=100, length=400)
        ...
        self.speed_lbl = ttk.Label(stats_frame, text="Speed: --")
        self.speed_lbl.pack(side=tk.LEFT)
        
        self.eta_lbl = ttk.Label(stats_frame, text="ETA: --")
        self.eta_lbl.pack(side=tk.RIGHT)

        ttk.Label(self.status_frame, text="Log:").pack(anchor=tk.W, pady=(10, 2))
        self.log_text = ScrolledText(self.status_frame, height=8, state=tk.DISABLED, font=("Consolas", 9))
